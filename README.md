...
| ----------------------------------- | ---------------------------------- |
| [1. 작성자](#작성자)                | [2. 기한](#기한)                   |
| [3. 사이트 기능](#작성자)           | [4. 사이트 사용법](#사이트-사용법) |
| [5. 동영상](#동영상)                | [6. 사이트](#사이트)               |
| [7. 프론트엔드](#프론트엔드)        | [8. 서버](#서버)                   |
| [9. 디비](#디비)                    | [10. 개발 환경](#개발-환경)        |
| [11. 도메인](#도메인)               | [12. 서버 호스팅](#서버-호스팅)    |
| [13. 데이터베이스](#데이터베이스)   | [14. 실행 장면](#실행-장면)        |
| [15. 프로젝트 구조](#프로젝트-구조) | [16. API 명세서](#API-명세서)      |
| [17. 참고 자료](#참고-자료)         | [18. 라이센스](#라이센스)          |
| [19. 만든 이유](#만든-이유)         | [20. 배운 점](#배운-점)            |
| [21. 확인 요망](#확인-요망)         | [22. 수정 요망](#수정-요망)        |
| [23. 디비 사용법](#디비-사용법)     | [24. 소스 사용법](#소스-사용법)    |
...
<pre>
1. 회원 가입(index.php)
   1) 이메일 인증(phpmailer 이메일 발신)
   2) 사진 용량 제한 및 크기 압축(자바스크립트로 처리)
   3) 회원 가입시 디비에 저장되는 unique_id 값을 세션으로 사용
</pre>

![화면 캡쳐](screenshot/signupPlay.gif)

<pre>
2. 로그인(views/login.php)
   1) 로그인 상태 유지(로그인에 이용한 email을 쿠키에 저장, 다음 로그인시 쿠키에 저장된 email 값을 바탕으로 자동으로 로그인 폼 채움)
   2) 비밀번호 찾기(이름과 이메일을 바탕으로 디비에서 검색 후 해당 데이터가 존재시 이메일 인증 후 비밀번호 알려줌)
</pre>

![화면 캡쳐](screenshot/login.gif)
![화면 캡쳐](screenshot/passwordFind.gif)

<pre>
3. 채팅 상대 검색(views/users.php)
   1) 채팅 상대 이름을 검색하면 해당 당사자가 화면에 나타나고 선택 후 채팅 가능
</pre>

![화면 캡쳐](screenshot/chatPlayerSearch.gif)

<pre>
4. 채팅 상대 선택(views/users.php)
   1) 화면 목록에 있는 채팅 상대자를 스크롤로 찾아가며 선택 가능
</pre>

![화면 캡쳐](screenshot/chatPlayerSelect.gif)

<pre>
5. 프로필 수정(views/edit-profile.php)
</pre>

![화면 캡쳐](screenshot/editProfile.gif)

<pre>
6. 로그아웃(user/logout.php)
</pre>

![화면 캡쳐](screenshot/logout.gif)

<pre>
7. 채팅(views/chat.php)
</pre>

![화면 캡쳐](screenshot/chat.gif)
...
[유튜브 영상](https://youtu.be/d1wjxqq0PQ0)🔗

### 사이트

[실제 사이트](https://chosangho.site/) 🔗
...
라이브러리: jquery, font-awesome(아이콘), emojionearea(이모티콘), SweetAlert(팝업)
</pre>
...
1. DB connection을 사용 후 끊어주지 않고 여러 PHP 파일에서
  ...
  (DB 커넥션을 끊어주지 않아도 PHP 가비지 콜렉터가 알아서 처리해준다고 알고 있었는데
   채팅 데이터를 비동기로 500ms 단위로 계속 데이터를 요청해서 그런가 디비 커넥션을 끊어주지 않고
   게속 요청이 일어나다 보니 디비 커넥션을 끊어주지 않으면 로딩이 걸려버려 데이터 받아오는게 느려짐)
...
2. 스마트폰, 태블릿 등 모바일 실제 기기에서 어떻게 CSS가 동작하는지 확인
</pre>
...
6. API를 REST API의 규칙대로 재구성, 해당 코드 리펙터링 필요함.

7. MVC 패턴에 맞게 코드 분배 후 재구성 및 수정 필요함.

8. mysql 각 테이블 정규화 규칙에 맞게 재설계 및 수정 필요함

9. 객체지향 SOLID 원칙을 고려해서 재설계 및 코드 수정 필요함

10. 주석 안 달은 부분 및 수정된 사항들 정리해서 주석 달기
